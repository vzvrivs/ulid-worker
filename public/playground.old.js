...
function getTimestampString() {
  const now = new Date();
  return now.toISOString().replace(/[:T]/g, "-").split(".")[0]; // ex: 2024-04-13-21-40-12
}
...
window.downloadAutofill = () => {
  ...
  a.href = url;
  a.download = `autofill_export_${getTimestampString()}.${format}`;
...
window.downloadConverted = () => {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `ulid_export_${getTimestampString()}.${format}`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
...
window.validateULID = async () => {
  ...
  try {
    ...
    output.textContent = JSON.stringify({ ulid, valid: false }, null, 2);
